  ...
  ******************************************************************************************
  <summary>
    ...
    Purpose:
        ...
        with ``@tool( parse_docstring=True )`` so LangChain can infer the tool input schema from
        callable signatures and type annotations, parse Google-style argument descriptions into
        the generated schema, execute the delegated Fonky implementation, and return its result
        through a LangChain agent or tool-calling workflow.
  </summary>
  ******************************************************************************************
'''
...
def load_google_speech_to_text( project_id: str, file_path: str,
    ...
    """Transcribe audio with Google Speech-to-Text.
        ...
        Args:
            ...
            speech_config (Optional[Dict[str, Any]]): Optional Google Speech-to-Text provider configuration mapping. This LangChain-facing name avoids the reserved ``config`` argument used internally for RunnableConfig injection.
