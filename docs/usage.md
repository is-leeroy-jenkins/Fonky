...
existing Fonky functions and methods into structured tools, exporting provider-compatible schemas,
executing tool calls, and returning JSON-safe results.

Fonky's AI tooling functionality is centered on `ToolDef` in `models.py`.

## What the AI Tooling Layer Does

Fonky does not require every loader, fetcher, scraper, or processor to be rewritten for AI use.
Instead, it wraps existing Python callables as structured tools.

The tooling flow is:

```text
Python function or object method
    -> ToolDef
...
| Serialize results   | Converts documents, dictionaries, lists, models, and primitive values into JSON-safe structures.  |

## Minimal Tool from a Plain Function
...
def summarize_text(text: str, max_words: int = 50) -> str:
    ...
    Purpose:
        Provides a deterministic function that can be exposed as an AI-callable tool.
...
## Tool from a Fonky Loader Method
...
Serialized document output should include:
...
## Tool from a Document Split Method

A loader instance can also expose a split method after documents are loaded.
...
def count_words(text: str) -> int:
    ...
    Purpose:
        Provides a deterministic utility that can be exposed to an AI agent.
...
## OpenAI-Compatible Schema Shape

`to_openai()` returns a schema shaped like this:
...
## Gemini-Compatible Schema Shape

`to_gemini()` returns a function declaration shaped like this:
...
## Grok-Compatible Schema Shape

`to_grok()` follows the function-tool shape used by `to_openai()`.
...
This is the core integration point between an AI model's tool-selection response and Fonky's Python
execution layer.

## Minimal Agent-Style Loop
...
## Tooling Pattern for Loader and Processor Workflows
...
| Document-like object          | Dictionary with `page_content` and `metadata` |
...
| Avoid destructive side effects | Do not expose destructive actions unless explicitly controlled.   |
